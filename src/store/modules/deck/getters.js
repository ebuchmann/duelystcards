...
const getCardText = card =>
  `${card.name} ${card.type} ${card.cost} ${card.rarity} ${card.text} ${
    card.race
  } ${card.set} ${card.faction}`.toLowerCase();
...
module.exports = {
  filteredCards: (state, getters, rootState) => {
    let cards = state.general
      ? [
        ...generals.filter(card => card.faction === state.general.faction),
        ...allCards[state.general.faction],
        ...allCards.neutral
      ]
      : generals;
    ...
    if (typeSelect.length) {
      cards = cards.filter(card => typeSelect.includes(card.type));
    }
    if (raritySelect.length) {
      cards = cards.filter(card => raritySelect.includes(card.rarity));
    }
    if (factionSelect.length) {
      cards = cards.filter(card => factionSelect.includes(card.faction));
    }
    if (tribeSelect.length) {
      cards = cards.filter(
        card => card.race && tribeSelect.includes(card.race)
      );
    }
    if (manaSelect.length) {
      cards = cards.filter((card) => {
        if (manaSelect.includes(9)) {
          return card.cost >= 9 || manaSelect.includes(card.cost);
        }
        return manaSelect.includes(card.cost);
      });
    }
    if (textSearch) {
      cards = cards.filter(card => filter(getCardText(card), textSearch));
    }
  ...
  factionCards: state =>
    sortBy(state.cards.filter(card => card.faction !== 'neutral'), [
      'cost',
      'name'
    ]),
